...
const App = () => {
  ...
  return (
    <div ref={containerRef} className={`flex flex-col h-dvh ${theme === 'dark' ? 'bg-[#020617] text-slate-100' : 'bg-slate-50 text-slate-900'} overflow-hidden font-sans relative transition-all duration-700`}>
      ...
      {/* Navbar - Restored Fullscreen Toggle for all devices */}
      <nav className={`fixed top-4 left-4 right-4 h-14 md:h-16 flex items-center justify-between px-3 md:px-6 z-[100] ${glassPanel} rounded-full mx-auto max-w-6xl`}>
        ...
        <div className="flex items-center gap-1 md:gap-2">
            {/* Mirror Type Segmented Control */}
            <div className={`flex p-0.5 md:p-1 rounded-full ${theme === 'dark' ? 'bg-black/30' : 'bg-slate-200/50'}`}>
                <button onClick={() => setMirrorType('concave')} className={`px-2.5 md:px-5 py-1 text-[8px] md:text-[10px] font-black rounded-full transition-all ${mirrorType === 'concave' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-400'}`}>CCV</button>
                <button onClick={() => setMirrorType('convex')} className={`px-2.5 md:px-5 py-1 text-[8px] md:text-[10px] font-black rounded-full transition-all ${mirrorType === 'convex' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-400'}`}>CVX</button>
            </div>
            
            {/* Theme Toggle */}
            <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className={`w-9 h-9 md:w-11 md:h-11 rounded-full flex items-center justify-center transition-all ${theme === 'dark' ? 'hover:bg-white/10' : 'hover:bg-black/5'}`} title="Toggle Theme">
              {theme === 'dark' ? <Sun className="w-4 h-4 text-amber-400" /> : <Moon className="w-4 h-4 text-blue-600" />}
            </button>

            {/* Fullscreen Toggle - Always visible for quick access */}
            <button onClick={toggleFullScreen} className={`w-9 h-9 md:w-11 md:h-11 rounded-full flex items-center justify-center transition-all ${theme === 'dark' ? 'hover:bg-white/10' : 'hover:bg-black/5'}`} title="Toggle Fullscreen">
              <Maximize2 className={`w-4 h-4 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`} />
            </button>
        </div>
      </nav>
      ...
      <div className="flex flex-1 overflow-hidden relative pt-20 md:pt-24">
        ...
        `}>
          <div className="p-6 md:p-8 space-y-6 md:space-y-10 flex-1 h-full flex flex-col overflow-y-auto custom-scrollbar">
            <div className="flex items-center justify-between border-b border-white/5 pb-4">
               ...
               <button onClick={() => setSidebarOpen(false)} className="p-2 bg-white/5 rounded-full hover:bg-white/10 transition-colors"><ChevronLeft className="w-5 h-5" /></button>
            </div>
            ...
            <section className={`p-5 rounded-[28px] border transition-all duration-500 mt-auto ${theme === 'dark' ? 'bg-blue-600/5 border-blue-500/20' : 'bg-blue-50 border-blue-100'}`}>
              ...
              <div className="space-y-3">
                ...
                {/* Math Table Integrated for Mobile */}
                {isMobile && (
        ...
        {!sidebarOpen && (
          <button onClick={() => setSidebarOpen(true)} className={`fixed left-4 md:left-8 top-1/2 -translate-y-1/2 z-[60] w-12 h-12 md:w-14 md:h-14 rounded-full border flex items-center justify-center transition-all duration-500 shadow-xl ${theme === 'dark' ? 'bg-slate-900 border-white/10 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
        ...
        <main className="flex-1 relative touch-none overflow-hidden" style={{ backgroundColor: colors.bg }}>
          ...
          {/* Floating Action Buttons - Reset Zoom/Pan */}
          <div className="absolute top-4 right-4 flex flex-col gap-2 md:gap-3 z-50">
             <button 
               onClick={() => {setZoom(1.0); setPanOffset({x:0, y:0});}} 
               className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-all ${glassPanel} border-none shadow-lg hover:scale-110 active:scale-95`}
               title="Reset View"
             >
...
const ToggleButton = ({ label, active, onClick, color, theme }) => {
  ...
  return (
    <button 
      onClick={onClick} 
      className={`flex items-center justify-between px-3 md:px-4 py-2.5 md:py-3.5 rounded-[16px] md:rounded-[20px] border transition-all duration-300 text-[9px] md:text-[10px] font-black uppercase tracking-[0.1em] ${active ? activeClass : inactiveClass} hover:scale-105 active:scale-95`}
    >
